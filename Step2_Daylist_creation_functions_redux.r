...
################ This version uses a reduced dataset without duplicates, internships or staffing agencies
###############################################################
...
for (year in years) { 
  tmp <- read.fst(paste0(path,"OJVsample_step1_redux_", year, ".fst"), as.data.table = TRUE, columns = c("general_id", "grab_date", "expire_date"))
...
dayx <- function(x, data, days, dsave, dreturn) {
  ...
  dayxdf <- data[grab_date <= x & grab_date > (x - days) & (expire_date > x | is.na(expire_date))]
...
saveRDS(daylist, file = paste0(path,"daylist_step3_30d_redux_",i, ".rds"),  compress = TRUE)
...
for (i in years) {
  ...
  saveRDS(daylist, file = paste0(path,"daylist_step3_20d_redux_",i, ".rds"),  compress = TRUE)
...
for (i in years) {
  ...
  saveRDS(daylist, file = paste0(path, "daylist_step3_40d_redux_",i, ".rds"),  compress = TRUE)
